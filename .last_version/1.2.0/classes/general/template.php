...
class WappiTemplate
{
  ...
  public static function GetList($aSort = array(), $arFilter = array())
  {
    ...
    foreach ($aSort as $key => $ord) {
      ...
      switch ($key) {
        case "ID":
          $arOrder[$key] = "T.ID " . $ord;
          break;
        case "TIMESTAMP_CREATE_X":
          $arOrder[$key] = "T.TIMESTAMP_CREATE_X " . $ord;
          break;
        case "TIMESTAMP_CHANGE_X":
          $arOrder[$key] = "T.TIMESTAMP_CHANGE_X " . $ord;
          break;
        case "ACTIVE":
          $arOrder[$key] = "T.ACTIVE " . $ord;
          break;
      }
    }
    if (count($arOrder) <= 0) {
